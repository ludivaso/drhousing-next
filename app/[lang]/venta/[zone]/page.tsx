...
export async function generateMetadata({
  ...
  const title = `Propiedades en Venta en ${zone.nameEs} | DR Housing`
  const description = zone.descriptionEs.slice(0, 160)
  const canonical = `https://drhousing.net/${lang}/venta/${zone.slug}`
  const altEn     = `https://drhousing.net/en/for-sale/${zone.slug}`
...
export default async function VentaZonePage({
  params,
}: {
  params: { lang: string; zone: string }
}) {
  ...
  // Redirect English visitors to the /for-sale/ route
  if (lang === 'en') {
    redirect(`/en/for-sale/${params.zone}`)
  }
  ...
  const jsonLd = {
    ...
    name: `Propiedades en Venta en ${zone.nameEs}`,
    description: zone.descriptionEs.slice(0, 300),
    url: `https://drhousing.net/es/venta/${zone.slug}`,
    address: {
      '@type': 'PostalAddress',
  ...
  return (
    <>
      ...
      />
      <ZoneLandingPage zone={zone} properties={properties} mode="sale" lang="es" />
    </>
  )
}
